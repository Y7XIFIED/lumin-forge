  ...
  const makeParticles = (element: HTMLElement) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => particle.parentNode?.removeChild(particle), t);
      }, 30);
    }
  };
      ...
      <div className="relative" ref={containerRef}>
        <nav className="flex relative" style={{ transform: 'translate3d(0,0,0.01px)' }}>
          <ul
            ...
            style={{ '--color-1': '#ff2a2a', '--color-2': '#fff', '--color-3': '#9f1239', '--color-4': '#f97316' } as React.CSSProperties}
          >
            {items.map((item, index) => (
              ...
              >
                <a href={item.href} className="outline-none py-[0.6em] px-[1em] inline-block">{item.label}</a>
              </li>
            ))}
          </ul>
        </nav>
