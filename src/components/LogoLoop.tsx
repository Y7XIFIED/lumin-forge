  ...
  ({
    ...
    const renderLogoItem = useCallback(
      (item: LogoItem, key: React.Key) => {
        if (renderItem) {
          return (
            <li className={cx('flex-none text-[length:var(--logoloop-logoHeight)] leading-[1]', isVertical ? 'mb-[var(--logoloop-gap)]' : 'mr-[var(--logoloop-gap)]')} key={key} role="listitem">
              {renderItem(item, key)}
            </li>
          );
        }
        ...
        const content = isNodeItem ? (
          <span className={cx('inline-flex items-center', scaleOnHover && 'transition-transform duration-300 group-hover/item:scale-120')}>
            {(item as any).node}
          </span>
        ) : (
          <img className={cx('h-[var(--logoloop-logoHeight)] w-auto block object-contain', scaleOnHover && 'transition-transform duration-300 group-hover/item:scale-120')} src={(item as any).src} alt={(item as any).alt ?? ''} loading="lazy" decoding="async" draggable={false} />
        );

        const inner = (item as any).href ? (
          <a className="inline-flex items-center no-underline rounded hover:opacity-80" href={(item as any).href} target="_blank" rel="noreferrer noopener">
            {content}
          </a>
        ) : (
          content
    ...
    return (
      <div ref={containerRef} className={rootClasses} style={containerStyle} role="region" aria-label={ariaLabel}>
        ...
        <div
          className={cx('flex will-change-transform select-none relative z-0', isVertical ? 'flex-col h-max w-full' : 'flex-row w-max')}
          ref={trackRef}
